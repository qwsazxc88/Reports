...
--функци€ возвращает строку с замещенными сотрудниками в штатной единице дл€ данного сотрудника
CREATE FUNCTION dbo.fnGetReplacedName
(
	--дл€ запуска функции нужно указать один из параметров
	@LinkId int	--Id св€зи штатной единицы и сотрудника
	,@ReplacedId int	--Id сотрудника который в отпуске, но его никто не замещает
	,@Switch int	--переключатель источника дл€ поиска записей
			--1 - ќ∆
			--2 - временные перемещени€
...
DECLARE 
	...
	IF @Switch = 1
	BEGIN
		--определ€ем кого замещает сотрудник
		...
		BEGIN
			...
			--пока цепл€емс€ отпускам по уходу за ребенком
			...
			--ORDER BY A.Id desc
		END
		ELSE	--определ€ем сотрудника, который ушел в отпуск по уходу за ребенком, но должность его свободна
			SELECT @ReplacedName += case when len(isnull(@ReplacedName, N'')) = 0 then N'' else N'; ' end + 
	...
	BEGIN
		...
		FROM (--замены через за€вки
					...
					UNION ALL
					--замены старые, которых в за€вках нет
