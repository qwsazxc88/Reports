...
--функци€ достает штатную расстановку по выбранному подразделению + текущее состо€ние надбавок 
...
DECLARE 
	...
	SELECT F.Id, A.Id as SEPId, A.PositionId, B.Name as PositionName, A.DepartmentId, 1 as Quantity
				 ...
				 case when (case when (isnull(E.IsPregnant, 0) = 1 or F.UserId is null) and isnull(F.ReserveType, 0) = 0 then 1 else 0 end) = 1 
										or (case when isnull(F.DocId, 0) = 0 then 0 else 1 end) = 1
							...
							then (case when (case when F.UserId is null then 0 else (case when isnull(E.IsPregnant, 0) = 1 or H.Id is not null then 1 else 0 end) end) = 1 
															or exists(SELECT * FROM StaffTemporaryReleaseVacancyRequest WHERE UserLinkId = F.Id and IsUsed = 1)
												 then '¬ременна€ ваканси€' else '¬аканси€' end) 
				 ...
				 ,case when F.ReserveType = 1 then N'ѕеремещение' 
							 ...
							 when F.ReserveType = 3 then N'—окращение' end as Reserve
				 ,F.DocId
	...
	--за€вки н создание временной вакансии при длительных отсутстви€х
	/*
