...
{
    public class NDFLPresenter : AbstractPresenter<INDFLView>
	{
		#region Fields
		...
		{
			base.OnInitialize();
			ViewNDFLAction action = new ViewNDFLAction();
		...
		{
			int count;
			IList<CalcListDto> list = DocumentDao.FindByFilter(new CalcListFilter(ReportType.NDFL,View.UserId.Value,
